...
/**
 * Controlador de Ventana Residentes
 * @author Alexander Nieves
 */
public class ResidentesController implements Initializable {

    @FXML
    private TableView<Residente> tableResidentes;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        String[] head = CONSTANT.RESIDENTES_HEADER.split(";");
        String[] cellValues =("cedula;nombre;telefono;email;estado;manzana;villa;habitantesWithRes;urbanizacion").split(";");
        
        for(int i=0; i<head.length; i++){
            
            tableResidentes.getColumns().add(new TableColumn(head[i]));
            tableResidentes.getColumns().get(i).setCellValueFactory(new PropertyValueFactory(cellValues[i]));
        }
        tableResidentes.getItems().setAll(Sistema.residentes);
