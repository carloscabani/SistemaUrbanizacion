...
public class ColaboradoresController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        String[] header = CONSTANT.COLABORADORES_HEADER.split(";");
        String[] cellValues = ("cedula;nombre;telefono;email;estado;puestoTrabajo;empleo;fechaInicio;fechaFin").split(";");
        for(int i = 0; i < header.length; i++){
            tableColaboradores.getColumns().add(new TableColumn(header[i]));
            tableColaboradores.getColumns().get(i).setCellValueFactory(new PropertyValueFactory(cellValues[i]));
        }
        tableColaboradores.getItems().setAll(Sistema.colaboradores);
